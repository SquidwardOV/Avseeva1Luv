...
1) Операции поиска вставки и удаления выполняются за константное время.
...
h_i(k)=(k%m)+i*(1+k%(m-2))%size //более подробная запись хэш-функции, записанной в предыдущей строчке.
h_0(k) — первоначальная хэш-функция (первое слагаемое у более подробной записи хэш-функции).
